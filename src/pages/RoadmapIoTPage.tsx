...
import roadmapData from "@/assets/iot-transformation-roadmap.json";

const RoadmapIoTPage = () => {
  const progressPercentage = 20; // Example progress
  const [selectedLevel, setSelectedLevel] = useState<{ id: string; name: string } | null>(null);

  // Function to get icon based on level content
        ...
        >
          ...
          <div className="flex flex-wrap justify-center gap-4 mt-8">
            ...
            <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-full shadow-sm border">
              ...
              <span className="text-sm font-medium">Modalidad: {roadmapData.modality}</span>
            </div>
            <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-full shadow-sm border">
              ...
              <div key={level.id} className="relative">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                  ...
                  {level.complementary && (
                    ...
                    >
                      <Card className="shadow-md hover:shadow-lg transition-shadow border border-gray-200 bg-gradient-to-br from-teal-50 to-cyan-50">
          ...
          <Card className="bg-gradient-to-r from-teal-50 to-cyan-50 border-0 shadow-lg">
            <CardContent className="p-8">
              <h2 className="text-2xl font-bold text-gray-900 mb-6 text-center">
                Caminos Complementarios de Especialización
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {roadmapData.complementaryPaths.map((path, index) => (
                  <motion.div
                    key={index}
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.5, delay: 1.3 + index * 0.1 }}
                    className="bg-white p-4 rounded-lg border border-teal-200 hover:shadow-md transition-shadow"
                  >
                    <h4 className="font-semibold text-gray-900 mb-2 text-sm">{path.title}</h4>
                    <Badge variant="outline" className="mb-3 text-xs bg-teal-50">
                      {path.durationWeeks} semanas
                    </Badge>
                    <p className="text-xs text-gray-600">{path.outcome}</p>
                  </motion.div>
                ))}
              </div>
            </CardContent>
          ...
          <Card className="bg-gradient-to-r from-teal-600 to-cyan-600 text-white border-0 shadow-xl">
            <CardContent className="p-8 text-center">
              ...
              <p className="text-teal-100">
                Obtén un reconocimiento oficial que validará tu experiencia en el diseño e implementación de soluciones IoT
              </p>
            </CardContent>
          </Card>
        </motion.div>
